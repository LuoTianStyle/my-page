  ...
  <div class="contentBar">
    ...
    <div class="content">
      <div
        ...
        :key="index"
        :style="{ top: item.top, left: item.left, zIndex: 1 }"
      >
        <div
          class="data"
          :style="{ backgroundImage: 'url(' + item.imgUrl1 + ')', backgroundRepeat: 'no-repeat' }"
        >{{ item.data }}</div>
        <div
          class="itemContent"
          :style="{ backgroundImage: 'url(' + item.imgUrl2 + ')', backgroundRepeat: 'repeat-y' }"
        >{{ item.content }}</div>
        <div
          class="bottom"
          :style="{ backgroundImage: 'url(' + item.imgUrl3 + ')', backgroundRepeat: 'no-repeat' }"
        >
    ...
    drag(el) {
      el.onmousedown = function(e) {
        ...
        document.onmousemove = function(e) {
          let oH = document.getElementsByClassName("contentBar")[0]
            .clientHeight;
...
.contentBar {
  ...
  .dialogBox {
    /deep/.el-dialog {
      ...
      box-shadow: 0 0.2rem 0.35rem 0 rgba(0, 0, 0, 0.3);
      width: 5rem;
      ...
      .submit {
        ...
        width: 1rem;
        height: 0.4rem;
        background: -webkit-linear-gradient(to left, hsla(40, 33%, 60%, 0.99), #c1866a 100%);
        background: linear-gradient(to left, hsla(40, 33%, 60%, 0.99), #c1866a 100%);
        border-radius: 0.2rem;
        font-size: 0.14rem;
        color: #fff;
        border: none;
